...
async def qsign_livecheck():
    msg_ids = []
    try:
        msg_id = (await nonebot.get_bot().send_group_msg(group_id=696748432,message='bot存活检测中,若此条未撤回,则bot进行一次重启'))["message_id"]
        ...
        try:
            ...
            asyncio.sleep(30)
            await nonebot.get_bot().send_group_msg(group_id=696748432,message='重启,上次下线原因为: '+str(e))
        except Exception:
