...
re = on_command('运气检测',aliases={'luck','随机壁纸','wpwp'}, priority=5, block=True)
...
async def _handle(matcher: Matcher,event: MessageEvent,bot:Bot):
    ...
    try: 
        await wp.send("您点的图来了{}".format(url)+MessageSegment.image(file=url, cache=False), at_sender=True)
    except Exception as e:
        await wp.finish('tx风控了...'+MessageSegment.image(file=erro, cache=False), at_sender=True)
    pass
...
async def _handle(matcher: Matcher,event: MessageEvent,bot:Bot):    
    ...
    try: 
        await re.send("看看你的运气{}".format(url)+MessageSegment.image(file=pic, cache=False), at_sender=True)
    except Exception as e:
...
async def _handle(state: T_State,matcher: Matcher,event: MessageEvent):    
    ...
    #await re.send("haha我不给")
    await search.finish('这是您要的{}'.format(aim)+MessageSegment.image(file=url, cache=False), at_sender=True)
